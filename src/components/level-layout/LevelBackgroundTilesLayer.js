import { THEME_TILES_MAP } from '../../helpers/const.js';
...
export default function LevelBackgroundTilesLayer({level, image}) {
    ...
    for (let y=0; y<= heightWithWalls; y++) {
        for (let x=0; x<= widthWithWalls; x++) {
            // Skip Bottom Left and Bottom Right for intentional blank tile
            if (y === heightWithWalls) {
            ...
            canvases.push(
            <MapCell key={`${x}_${y}`} x={x} y={y} frameCoord={getBackgroundTile(x, y)} image={image}/>
                );
        }
    }
